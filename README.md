...
It will invoke a chat loop which will end when report is done (see step 4)
...
You're not have to provide all the answers for an entire chat script - For example, if you provide array with 6 cells (6 answers), the chat flow layer will answer questions 7 & up randomly. Addionally, providing a `null` as an asnwer will also trigger a random answer. For exampl, If you only care about asnwers 5-6, you can provide asnewr array liek the following:
```
